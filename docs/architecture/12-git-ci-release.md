...
CI publishes immutable registry artifacts (`web`, `api`, `worker`, `docs`). Deploying those
SHA tags to a host is **bring-your-own CD** — see [docs/runbooks/deploy.md](../runbooks/deploy.md).
Migrate is the api image with
...
Image coordinates: `ghcr.io/<owner>/{web,api,worker,docs}:<git-sha>`. Owner is lowercased. Local
