...
# One-shot migrate job for @repo/db — turbo prune + tsdown bundle.
# Run: `node dist/migrate.mjs` (SQL at /app/migrations).
#
# Migrations deliberately do not ride inside an application image: the deploy
# sequence is migrate-then-roll, and coupling the two means a rollback of the
# app is not a rollback of the schema.
...
# Strip sourcemaps from the runtime layer so they never ship in the image.
RUN pnpm --filter @repo/db build \
  && find packages/db/dist -type f -name '*.map' -delete
